...
<body style="background-image:url(img/bjfu<?php if($gend==1){p(2);}?>.jpg);">
...
<div class="judgebox">
	<div class="left">
		...
		<a class="btn<?php p($gend==0?'2':'1')?>" bjfu="sbtn" id="leftbtn" obj="uid1"></a>
	</div>
	<div class="right">
		...
		<a class="btn<?php p($gend==0?'2':'1')?>" bjfu="sbtn" id="rightbtn" obj="uid2"></a>
	</div>
	<input type="hidden" id="verify">
	<input type="hidden" id="grade" value="0">
	<div class="equal">
		<div class="f14 b refresh" bjfu="refresh">很难抉择，换一换</div>
		<div class="keycon f12">可以使用快捷键投票 H 左边 / L 右边 / X 战平</div>
		<div class="keycon f12" bjfu="grade">筛选年级 
			<select id="grades">
...
<script>
$("[bjfu='grade']").change(function(){
		$("#grade").val($("#grades").val());
		$("[bjfu='sbtn']").trigger("click");
	});
